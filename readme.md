...
Введите токен refresh в поле "Refresh Token" и нажмите кнопку обновить токен, после чего вернется новый access токен    
    
Для тестирования функционала: просмотра профиля и принятия инвайт кода от другого пользователя удобно использовать приложение Postman или аналогичную программу    
- Включите access токен доступа в заголовок запроса: Authorization - Bearer <access_token>    
- Установите в заголовке запроса: Accept - text/html, чтобы возвращалась html страница, а не json

### 4. Для просмотра профиля пользователя     
Введите в строке запроса: http://<IP-адрес вашего сервера>:8000/users/retrieve/    
   
### 5. Принимаем инвайт код другого пользователя   
Введите в строке запроса: http://<IP-адрес вашего сервера>:8000/users/set_referrer/    
В поле "Введите инвайт-код": введите инвайт-код, полученный вами от реферера, после чего он занесется в БД, а Вам вернется сообщение: "Вы стали рефералом пользователя с инвайт-кодом cAXcJR"   
или другая информация об ограничениях:   
- "Вы уже являетесь рефералом пользователя с инвайт-кодом cAXcJR"   
